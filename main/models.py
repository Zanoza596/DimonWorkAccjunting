...
class OperationsCategories(models.Model):
    name=models.CharField(max_length=150,blank=True,null=True,verbose_name='Название')
    #Operations=models.PositiveIntegerField()
    parent=models.ForeignKey(to='self',null=True, verbose_name='Родмтель', on_delete=models.CASCADE)#Отображает множество сущностей ComponentTechCategorys на себя
    note=models.TextField(max_length=200,blank=True,null=True,verbose_name='Примечания')
    slug=models.SlugField(max_length=200,unique=True,blank=True,null=True,verbose_name='URL')
...
class ComponentsGoals(models.Model):
    name=models.CharField(max_length=150,blank=True,null=True,verbose_name='Название')
    #Projects=models.PositiveIntegerField()
    parent=models.ForeignKey(to='self',null=True, verbose_name='Родмтель', on_delete=models.CASCADE)#Отображает множество сущностей ComponentGoals на себя
    note=models.TextField(max_length=200,blank=True,null=True,verbose_name='Примечания')
    slug=models.SlugField(max_length=200,unique=True,blank=True,null=True,verbose_name='URL')
...
class ComponentsComponents(models.Model):
    ...
    ownerComponent=models.ForeignKey(to=Components, null=True, related_name='parts', verbose_name='Владелец', on_delete=models.CASCADE)
    partComponent=models.ForeignKey(to=Components,null=True, related_name='assembles', verbose_name='Элемент', on_delete=models.CASCADE)
...
class Operations(models.Model):
    ...
    owners=models.ManyToManyField(to="Operations",through="OperationsOperations",through_fields=("ownerOperation","partOperation"), verbose_name='Владелец')#Отображает множество сущностей Components на себя(владельцы)
    time=models.TimeField(blank=True,null=True,verbose_name='Время')
    unit=models.ForeignKey(to=Units,blank=True,null=True, verbose_name='Елиница', on_delete=models.CASCADE)
    cost=models.DecimalField(default=0.00,max_digits=7,decimal_places=2,verbose_name='Стоимость')
    energycost=models.DecimalField(default=0.00,max_digits=7,decimal_places=2,verbose_name='Затраты энергии')
    ...
    #unitTime=models.TimeField(verbose_name='Время ')
    salary=models.DecimalField(default=0.00,max_digits=7,decimal_places=2,verbose_name='Зарплата')
    consumables=models.ForeignKey(to=Components,blank=True,null=True, related_name='consumables_set', verbose_name='Расходные материалы', on_delete=models.CASCADE)
    #Childs если категория - root
    #ConsumptionComponents=models.PositiveIntegerField() расходные компоненты
    retreatRowMaterials=models.ForeignKey(to=Components,blank=True,null=True, related_name='retreatRowMaterials_set', verbose_name='Отходы', on_delete=models.CASCADE) 
    amortization=models.ForeignKey(to=Amortizations,blank=True,null=True, verbose_name='Амортизация', on_delete=models.CASCADE)
    #Works=models.PositiveIntegerField()
    note=models.TextField(max_length=200,blank=True,null=True,verbose_name='Примечания')
    slug=models.SlugField(max_length=200,unique=True,blank=True,null=True,verbose_name='URL')
...
class OperationsOperations(models.Model):

    ownerOperation=models.ForeignKey(to=Operations, null=True, related_name='parts', verbose_name='Надоперация', on_delete=models.CASCADE)
    partOperation=models.ForeignKey(to=Operations,null=True, related_name='assembles', verbose_name='Подоперация', on_delete=models.CASCADE)
    quantity=models.DecimalField(default=1.00,max_digits=7,decimal_places=2,verbose_name='Количество')
